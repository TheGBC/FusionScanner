...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
  ...
  using System.Text;
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        /// </summary>
        private float voxelsPerMeter = 384.0f;
        ...
        private Matrix4 xRotate(float rad) {
          Matrix4 r = Matrix4.Identity;

          r.M22 = (float) Math.Cos(rad);
          r.M23 = (float) (-1 * Math.Sin(rad));
          r.M33 = (float) Math.Cos(rad);
          r.M32 = (float) Math.Sin(rad);

          return r;
        }

        private Matrix4 yRotate(float rad) {
          Matrix4 r = Matrix4.Identity;

          r.M11 = (float)Math.Cos(rad);
          r.M31 = (float)(-1 * Math.Sin(rad));
          r.M33 = (float)Math.Cos(rad);
          r.M13 = (float)Math.Sin(rad);

          return r;
        }

        private Matrix4 zRotate(float rad) {
          Matrix4 r = Matrix4.Identity;
          r.M11 = (float)Math.Cos(rad);
          r.M12 = (float)(-1 * Math.Sin(rad));
          r.M22 = (float)Math.Cos(rad);
          r.M21 = (float)Math.Sin(rad);

          return r;
        }

        private Matrix4 multiply(Matrix4 m1, Matrix4 m2, Matrix4 m3) {
          return M3DtoM4(Matrix3D.Multiply(Matrix3D.Multiply(M4toM3D(m1), M4toM3D(m2)), M4toM3D(m3)));
        }

        private Matrix3D M4toM3D(Matrix4 m) {
      ...
      private Matrix4 M3DtoM4(Matrix3D m) {
        ...
        mat.M33 = (float) m.M33;
        mat.M34 = (float) m.M34;
        mat.M41 = (float) m.OffsetX;
        mat.M42 = (float) m.OffsetY;
        mat.M43 = (float) m.OffsetZ;
        mat.M44 = (float) m.M44;

        return mat;
      }

        private void write(float xRot, float yRot, float zRot, string name) {
          FusionPointCloudImageFrame frame = new FusionPointCloudImageFrame(this.downsampledWidth, this.downsampledHeight);

          this.volume.CalculatePointCloud(frame, multiply(xRotate(xRot), yRotate(yRot), zRotate(zRot)));

          float[] arr = new float[this.downsampledWidth * this.downsampledHeight * 6];
          frame.CopyPixelDataTo(arr);
          ...
          builder.Append(']');
          System.IO.File.WriteAllText(name, builder.ToString());  
        }
        ...
        {
            short[] outArr = new short[this.voxelsX * this.voxelsY * this.voxelsZ];
            this.volume.ExportVolumeBlock(0, 0, 0, this.voxelsX, this.voxelsY, this.voxelsZ, 1, outArr);
            StringBuilder builder = new StringBuilder();
            builder.Append('[');
            bool first = true;
            foreach (short s in outArr) {
              if (!first) {
                builder.Append(',');
              }
              first = false;
            ...
            builder.Append(']');
            System.IO.File.WriteAllText("out.txt", builder.ToString());
            this.cameraPoseFinder.SaveCameraPoseFinderDatabase("poseFinder.txt");

            FusionPointCloudImageFrame frame = new FusionPointCloudImageFrame(this.downsampledWidth, this.downsampledHeight);

            Matrix4 transform = Matrix4.Identity;
            this.volume.CalculatePointCloud(frame, Matrix4.Identity);
            ColorMesh mesh = this.volume.CalculateMesh(1);
            using (StreamWriter writer = new StreamWriter("model.obj")) {
